...
TerminalDisplay::TerminalDisplay(QWidget* parent) :
    QWidget(parent), _screenWindow(nullptr), _allowBell(true), _gridLayout(nullptr), _fontHeight(1), _fontWidth(1), _fontAscent(1), _boldIntense(true), _lines(1), _columns(1), _usedLines(1), _usedColumns(1), _contentHeight(1), _contentWidth(1), _image(nullptr), _randomSeed(0), _resizing(false), _terminalSizeHint(false), _terminalSizeStartup(true), _bidiEnabled(false), _mouseMarks(false), _disabledBracketedPasteMode(false), _actSel(0), _wordSelectionMode(false), _lineSelectionMode(false), _preserveLineBreaks(false), _columnSelectionMode(false), _scrollbarLocation(TTTermWidget::NoScrollBar), _wordCharacters(QLatin1String(":@-./_~")), _bellMode(SystemBeepBell), _scrollOnKeypress(true), _blinking(false), _hasBlinker(false), _cursorBlinking(false), _hasBlinkingCursor(false), _allowBlinkingText(true), _ctrlDrag(false), _tripleClickMode(SelectWholeLine), _isFixedSize(false), _possibleTripleClick(false), _flowControlWarningEnabled(false), _outputSuspendedLabel(nullptr), _lineSpacing(0), _colorsInverted(false), _opacity(static_cast<qreal>(1)), _backgroundMode(None), _filterChain(new TerminalImageFilterChain()), _cursorShape(Emulation::KeyboardCursorShape::BlockCursor), mMotionAfterPasting(NoMoveScreenWindow), _leftBaseMargin(1), _topBaseMargin(1), _drawLineChars(true) {
...
void TerminalDisplay::drawCharacters(QPainter& painter,
    ...
    if (font.bold() != useBold || font.underline() != useUnderline || font.italic() != useItalic || font.strikeOut() != useStrikeOut || font.overline() != useOverline) {
...
void TerminalDisplay::scrollImage(int lines, const QRect& screenWindowRegion) {
    ...
    if (lines == 0 || _image == nullptr || !region.isValid() || (region.top() + abs(lines)) >= region.bottom() || this->_lines <= region.height()) return;
...
void TerminalDisplay::paintFilters(QPainter& painter) {
    ...
    int cursorLine;
    int cursorColumn;
    int leftMargin = _leftBaseMargin + ((_scrollbarLocation == TTTermWidget::ScrollBarLeft && !_scrollBar->style()->styleHint(QStyle::SH_ScrollBar_Transient, nullptr, _scrollBar)) ? _scrollBar->width() : 0);
...
void TerminalDisplay::extendSelection(const QPoint& position) {
    ...
    if (_wordSelectionMode) {
        ...
        if (i >= 0 && i <= _imageSize) {
            ...
            while (((left.x() > 0) || (left.y() > 0 && (_lineProperties[left.y() - 1] & LINE_WRAPPED))) && charClass(QChar(_image[i - 1].character)) == selClass) {
                i--;
                if (left.x() > 0)
                    left.rx()--;
                else {
        ...
        if (i >= 0 && i <= _imageSize) {
            selClass = charClass(QChar(_image[i].character));
            while (((right.x() < _usedColumns - 1) || (right.y() < _usedLines - 1 && (_lineProperties[right.y()] & LINE_WRAPPED))) && charClass(QChar(_image[i + 1].character)) == selClass) {
                i++;
...
void TerminalDisplay::mouseReleaseEvent(QMouseEvent* ev) {
    ...
    if (!_mouseMarks &&
        ((ev->button() == Qt::RightButton && !(ev->modifiers() & Qt::ShiftModifier)) || ev->button() == Qt::MiddleButton)) {
...
void TerminalDisplay::mouseDoubleClickEvent(QMouseEvent* ev) {
    ...
    QChar selClass = charClass(QChar(_image[i].character));
    {
        ...
        x = endSel.x();
        while (((x < _usedColumns - 1) || (endSel.y() < _usedLines - 1 && (_lineProperties[endSel.y()] & LINE_WRAPPED))) && charClass(QChar(_image[i + 1].character)) == selClass) {
            i++;
            if (x < _usedColumns - 1)
                x++;
            else {
...
void TerminalDisplay::setSize(int columns, int lines) {
    int scrollBarWidth = (_scrollBar->isHidden() || _scrollBar->style()->styleHint(QStyle::SH_ScrollBar_Transient, nullptr, _scrollBar)) ? 0 : _scrollBar->sizeHint().width();
