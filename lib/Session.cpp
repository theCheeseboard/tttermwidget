...
Session::Session(bool connectPtyData, QObject* parent) :
    QObject(parent),
    _shellProcess(nullptr), _emulation(nullptr), _monitorActivity(false), _monitorSilence(false), _notifiedActivity(false), _autoClose(true), _wantedClose(false), _silenceSeconds(10), _isTitleChanged(false), _addToUtmp(false) // disabled by default because of a bug encountered on certain systems
...
bool Session::isBusy() {
    ...
    if (_shellProcess->processId() == _shellProcess->foregroundProcessGroup()) {
        return false;
    } else {
        return true;
    }
