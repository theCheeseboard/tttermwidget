...
QList<KeyboardTranslatorReader::Token> KeyboardTranslatorReader::tokenize(const QString& line) {
    ...
    static QRegularExpression title(QRegularExpression::anchoredPattern("keyboard\\s+\"(.*)\""));
    // key line: key KeySequence : "output"
    // key line: key KeySequence : command
    static QRegularExpression key(QRegularExpression::anchoredPattern("key\\s+([\\w\\+\\s\\-\\*\\.]+)\\s*:\\s*(\"(.*)\"|\\w+)"));
