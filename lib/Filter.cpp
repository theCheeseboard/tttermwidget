...
Filter::HotSpot::HotSpot(int startLine, int startColumn, int endLine, int endColumn) :
    _startLine(startLine), _startColumn(startColumn), _endLine(endLine), _endColumn(endColumn), _type(NotSpecified) {
}
...
void RegExpFilter::process() {
    ...
    if (QRegularExpression(QRegularExpression::anchoredPattern(_searchText.pattern())).match(emptyString).hasMatch())
        return;

    QRegularExpressionMatchIterator matches = _searchText.globalMatch(*text);
...
UrlFilter::HotSpot::HotSpot(int startLine, int startColumn, int endLine, int endColumn) :
    RegExpFilter::HotSpot(startLine, startColumn, endLine, endColumn), _urlObject(new FilterObject(this)) {
    setType(Link);
}
...
UrlFilter::HotSpot::UrlType UrlFilter::HotSpot::urlType() const {
    ...
    if (QRegularExpression(QRegularExpression::anchoredPattern(FullUrlRegExp.pattern())).match(url).hasMatch())
        return StandardUrl;
    else if (QRegularExpression(QRegularExpression::anchoredPattern(EmailAddressRegExp.pattern())).match(url).hasMatch())
        return Email;
    else
        return Unknown;
}
